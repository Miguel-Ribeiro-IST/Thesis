...
%{
for i=1:size(B1,1)
B1(i,3)=european_bs(S01,B1(i,2),r1,B1(i,3),B1(i,1)./252,'call');
end
%}
...
if dt*D*(k-1)<=times(i)
   ...
   Z=mean(Y);
   for i=1:size(B,1)
        euro=@(sigma)european_bs(S0,B(i,1),r,sigma,T,'call')-Z(i)*exp(-r*T);
        Z(i)=fzero(euro,0.5);
   end
   Error(iter)=sum(abs(Z-B(:,2)'));
   clear Y;
...
Y(i) = max(S(i)-K,0);
  ...
  euro=@(sigma)european_bs(S0,K,r,sigma,T,'call')-mean(Y)*exp(-r*T);
  imp_vol(iter)=fzero(euro,0.5);
end

Euro_final=mean(imp_vol);
end
