...
optimvars=Optimizer(beta,S0,P,r,x0,OptMethod);  %Obtain optimization variables
alpha=optimvars(1);
rho=optimvars(2);
nu=optimvars(3);
...
%scatter(ax2,B(:,2),P(:),'.');
hold on;
scatter(ax2,B(:,2),abs(Price(:)-P(:))./P(:),'x');
%set(gca,'yscale','log')
...
for i=1:size(B,1)
    ...
    Price=european_bs(S0,B(i,2),r,sigmaSABR(alpha,rho,nu,beta,B(i,2),S0*exp(r*B(i,1)),B(i,1)),B(i,1),"call");
    LS=LS+(((B(i,3)-Price)./B(i,3))^2);
