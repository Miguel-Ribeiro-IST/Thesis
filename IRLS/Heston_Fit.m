...
%%%%%%%%%%%%%%%%%%%%%    CALIBRATION      %%%%%%%%%%%%%%%%%%%%%%
x0=[10,0.2,0.2,-0.25,1.2];   %parameter starting values [kappa,nubar,nu0,rho,chi]
...
for i=1:iter
    vars=Optimizer(B,S0,r,x0,OptAlg,lb,ub,weights);
    weights=Reweight(vars(1),vars(2),vars(3),vars(4),vars(5),S0,B,r,lb,ub);
end
optimvars=vars;

f=Hestoncal(optimvars(1),optimvars(2),optimvars(3),optimvars(4),optimvars(5),S0,B,r,lb,ub,weights);
...
function weights=Reweight(kappa,nubar,nu0,rho,chi,S0,B,r,lb,ub)
errors=ones(size(B,1),1);
for i=1:size(B,1)
    errors(i)=(B(i,3)-HestonPrice(B(i,2),B(i,1),S0,r,kappa,nubar,nu0,rho,chi,"vol",lb,ub))^2;
end
...
function optimvars=Optimizer(P,S0,r,x0,OptAlg,lb,ub,weights)
tic
warning('off','all');

fun=@(var)Hestoncal(var(1),var(2),var(3),var(4),var(5),S0,P,r,lb,ub,weights); %function to be optimized.
...
function Total_Error=Hestoncal(kappa,nubar,nu0,rho,chi,S0,P,r,lb,ub,weights)
LS=0;
for i=1:size(P,1)
    ...
    LS=LS+weights(i).*(((P(i,3)-HestonPrice(P(i,2),P(i,1),S0,r,kappa,nubar,nu0,rho,chi,"vol",lb,ub))./P(i,3))^2);
