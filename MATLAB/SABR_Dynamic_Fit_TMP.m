...
S0=17099.4;
r = 0.06;
matur=1;           %maturity until which we want to fit the data.
...
for iter=1:matur
    ...
    SABRVol=@(K)sigmaSABR2(alpha,rho0,nu0,a,b,beta,K,S0*exp(r*T),T);
...
LS=0;
for i=1:size(B,1)
    LS=LS+(abs(B(i,3)-sigmaSABR2(alpha,rho,nu,a,b,beta,B(i,2),S0*exp(r*B(i,1)),B(i,1))));
end
Total_Error=LS;
end
end
