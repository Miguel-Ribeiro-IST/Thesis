...
%%{
sigma=Optimizer(P,S0,r,x0,OptAlg);
%}
...
function optimvars=Optimizer(P,S0,r,x0,OptAlg)


fun=@(var)Constcal(var(1),S0,P,r); %function to be optimized.
...
fprintf('sigma=%f\nerror=%f\n',[optimvars(1),Constcal(optimvars(1),S0,P,r)])
end



%%%  CALCULATE LEAST SQUARES ERROR BETWEEN MODEL AND DATA  IMPLIED VOL  %%%
function Total_Error=Constcal(sigma,S0,P,r)
...
for i=1:size(P,1)
    ...
    LS=LS+(P(i,3)-european_bs(S0,P(i,2),r,sigma,P(i,1),"call"))^2*(1-abs(P(i,2)-1))^2;
