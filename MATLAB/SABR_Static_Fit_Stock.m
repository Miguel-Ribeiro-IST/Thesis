...
if SimPoints
    for i=1:size(B,1)
        ...
        Volatility2(i)=Pricer2(alpha,rho,nu,beta,B(i,2),S0*exp(r*T),r,T,L,M,iterations,"vol");
...
function Result_Avg=Pricer2(alpha,rho,nu,beta,K,f,r,T,L,M,iterations,PriceVol)
dt = T/L;      %time steps
...
for iter=1:iterations    %Perform the "for" cycle in parallel
    alp=alpha*ones(M,1);    %define the initial vector of volatilities
    S=f*exp(-r*T)*ones(M,1);
    for k = 1:L
        Z1=randn(M,1);                                 %vector of random variables
        Z2=rho*Z1+sqrt(1-rho^2)*randn(M,1);                         %intermediate variable
        S(:)=max(S(:).*(1+r*dt)+sqrt(dt)*alp(:).*S(:).^beta.*Z1.*exp(-r*(T-dt*k)*(1-beta)),0);
        alp(:)=alp(:).*(1+nu.*sqrt(dt).*Z2);  %update volatilities vector
    end
