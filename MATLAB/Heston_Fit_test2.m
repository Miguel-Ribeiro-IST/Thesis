...
%fun=@(u)real(CharFuncHeston(u,1/12,S0,r,kappa,nubar,nu0,rho,chi));
%fplot(fun,[0.4,1.6])
T=10;
K=2;
nu0=0.16;
...
chi=2;
rho=-0.8;
fun1=@(u)real(exp(-1i.*u.*log(K))/(1i*u*CharFuncCrisos(-1i,T,S0,r,kappa,nubar,nu0,rho,chi)));
fun2=@(u)real(CharFuncCrisos(u-1i,T,S0,r,kappa,nubar,nu0,rho,chi));
fun3=@(u)real(exp(-1i.*u.*log(K))/(1i*u*CharFuncCrisos(-1i,T,S0,r,kappa,nubar,nu0,rho,chi))*CharFuncCrisos(u-1i,T,S0,r,kappa,nubar,nu0,rho,chi));

fun1=@(u)real(exp(-1i.*u.*log(K))/(1i*u));
fun2=@(u)real(CharFuncCrisos(u,T,S0,r,kappa,nubar,nu0,rho,chi));
fun3=@(u)real(exp(-1i.*u.*log(K))/(1i*u)*CharFuncCrisos(u,T,S0,r,kappa,nubar,nu0,rho,chi));
...
ax5=subplot(3,2,4);
ax6=subplot(3,2,6);
fplot(ax1,fun1,[0,100])
fplot(ax2,fun2,[0,100])
fplot(ax3,fun3,[0,100])
fplot(ax4,fun4,[0,100])
fplot(ax5,fun5,[0,100])
fplot(ax6,fun6,[0,100])

format long
HestonPrice(K,T,S0,r,kappa,nubar,nu0,rho,chi,"price",lb,ub)
...
P1=1/2+1/pi.*integral(fun1,0.00000001,100);
P2=1/2+1/pi.*integral(fun2,0.00000001,100);
...
function result=HestonPriceCrisos(K,T,S0,r,kappa,nubar,nu0,rho,chi,PriceVol,lb,ub)
warning('off','all');
...
fun1=@(u)real(exp(-1i.*u.*log(K))./(1i.*u.*CharFuncHeston(-1i,T,S0,r,kappa,nubar,nu0,rho,chi)).*CharFuncHeston(u-1i,T,S0,r,kappa,nubar,nu0,rho,chi));
fun2=@(u)real(exp(-1i.*u.*log(K))./(1i.*u).*CharFuncHeston(u,T,S0,r,kappa,nubar,nu0,rho,chi));
P1=1/2+1/pi.*integral(fun1,0.00000001,100);
P2=1/2+1/pi.*integral(fun2,0.00000001,100);
call=S0.*P1-exp(-r.*T).*K.*P2;
...
function w=CharFuncCrisos(u,T,S0,r,kappa,nubar,nu0,rho,chi)
a=-u^2/2-1i*u/2;
b=kappa-rho*chi*1i*u;
gamma=chi^2/2;
h=sqrt(b^2-4*a*gamma);
rp=(b+h)/chi^2;
rm=(b-h)/chi^2;
g=rm/rp;
D=rm*((1-exp(-h*T))/(1-g*exp(-h*T)));
C=kappa*(rm*T-2/chi^2*log((1-g*exp(-h*T))/(1-g)));
w=exp(C*nubar+D*nu0+1i*u*log(S0*exp(r*T)));
end
