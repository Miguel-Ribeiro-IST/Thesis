...
Many other diffusion processes exist, and this choice made by Black \textit{et al.} might seem a bit arbitrary, but there is actually a reason behind it. Changes in the stock price are geometric and not arithmetic. If an asset of price $\SI{1000}{\EUR}$ changed by $\SI{9}{\EUR}$, we would barely take notice. If another asset of price $\SI{10}{\EUR}$ changed by $\SI{9}{\EUR}$, the drop would be quite surprising, even though the change was the same in both assets. This seems to indicate that changes in the stock price depend on the stock price itself and justifies the choice made by Black \textit{et al.} to model stock prices using a Geometric Brownian Motion process.
\begin{figure}[!htb]
    \centering
...
\begin{equation}
C_{Barr}=S_0\left(N(d_3)+b\left(N(d_4)-N(d_5)\right)\right)-Ke^{-r(T-t)}\left(N(d_6)+a\left(N(d_7)-N(d_8)\right)\right),
\end{equation}
...
\end{equation}
\begin{equation}
d_3=\frac{\log(S_0/B)+(r+\frac{1}{2}\sigma^2)(T-t)}{\sigma\sqrt{T-t}},
\end{equation}
\begin{equation}
d_4=\frac{\log(S_0/B)-(r+\frac{1}{2}\sigma^2)(T-t)}{\sigma\sqrt{T-t}},
\end{equation}
\begin{equation}
d_5=\frac{\log(S_0K/B^2)-(r+\frac{1}{2}\sigma^2)(T-t)}{\sigma\sqrt{T-t}},
\end{equation}
\begin{equation}
d_6=\frac{\log(S_0/B)+(r-\frac{1}{2}\sigma^2)(T-t)}{\sigma\sqrt{T-t}},
\end{equation}
\begin{equation}
d_7=\frac{\log(S_0/B)-(r-\frac{1}{2}\sigma^2)(T-t)}{\sigma\sqrt{T-t}},
\end{equation}
\begin{equation}
d_8=\frac{\log(S_0K/B^2)-(r-\frac{1}{2}\sigma^2)(T-t)}{\sigma\sqrt{T-t}},
\end{equation}
