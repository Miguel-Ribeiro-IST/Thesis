...
r = 0.00000001;          %risk-free rate. Forward prices in data file assumed r=0.06
matur=8;           %maturity at which we want to fit the data. If matur=5, the fifth maturity in the file is chosen.
OptMethod="CMA";
iterations2=5;
...
optimvars=IRLSOptim(S0,B,r,x0,OptMethod,lb,ub,iterations2);  %Obtain optimization variables
alpha=optimvars(1);
rho=optimvars(2);
nu=optimvars(3);
beta=optimvars(4);
...
function optimvars=IRLSOptim(S0,B,r,x0,MultiStoch,lb,ub,iter)
weights=ones(size(B,1),1);
for i=1:iter
    vars=Optimizer(S0,B,r,x0,MultiStoch,lb,ub,weights);
    weights=Reweight(vars(1),vars(2),vars(3),vars(4),S0,B,r);
end
optimvars=vars;

f=SABRcal(optimvars(1),optimvars(2),optimvars(3),optimvars(4),S0,B,r,weights);
...
function weights=Reweight(alpha,rho,nu,beta,S0,B,r)
errors=ones(size(B,1),1);
for i=1:size(B,1)
    errors(i)=abs(B(i,3)-sigmaSABR(alpha,rho,nu,beta,B(i,2),S0*exp(r*B(i,1)),B(i,1)));
end

errors=errors./(max(errors));
weights=(1-errors.^3).^3;
...
function optimvars=Optimizer(S0,B,r,x0,MultiStoch,lb,ub,weights)
fun=@(var)SABRcal(var(1),var(2),var(3),var(4),S0,B,r,weights); %function to be optimized.
...
scatter(ax1,B(:,2),B(:,3),'.');
hold on;
fplot(ax1,SABRVol,[min(B(:,2)) max(B(:,2))])
...
function Total_Error=SABRcal(alpha,rho,nu,beta,S0,B,r,weights)
...
for i=1:size(B,1)
    ...
    LS=LS+weights(i).*(((B(i,3)-sigmaSABR(alpha,rho,nu,beta,B(i,2),S0*exp(r*B(i,1)),B(i,1)))./B(i,3))^2);
