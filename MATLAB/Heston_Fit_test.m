...
T=1/12;
K=1.2;
fun1=@(u)real(exp(-1i.*u.*log(K))./(1i.*u.*S0.*exp(r.*T)));
fun2=@(u)real(CharFuncHeston(u-1i,T,S0,r,kappa,nubar,nu0,rho,chi));
fun3=@(u)real(exp(-1i.*u.*log(K))./(1i.*u.*S0.*exp(r.*T)).*CharFuncHeston(u-1i,T,S0,r,kappa,nubar,nu0,rho,chi));

fun4=@(u)real(exp(-1i.*u.*log(K))./(1i.*u));
fun5=@(u)real(CharFuncHeston(u,T,S0,r,kappa,nubar,nu0,rho,chi));
fun6=@(u)real(exp(-1i.*u.*log(K))./(1i.*u).*CharFuncHeston(u,T,S0,r,kappa,nubar,nu0,rho,chi));
